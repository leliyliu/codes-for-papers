...
def main():
    ...
    parser.add_argument('--q-mode', choices=q_modes_choice, default='layer_wise',
                    help='Quantization modes: ' + ' | '.join(q_modes_choice) +
                            ' (default: kernel-wise)')
    parser.add_argument('--teacher', default='mobilenetv2', type=str, help='teacher model type')
    ...
    args.save = 'cifar100-kd-{}-{}-{}-{}'.format(args.net, args.save, args.quan_mode[-3:], time.strftime("%Y%m%d-%H%M%S"))
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        }, is_best, save='logger/kd-{}-{}-{}-models'.format('cifar100', args.net, args.quan_mode[-3:]))
